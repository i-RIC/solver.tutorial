...
!  Last updated (mm-dd-yyyy): 06-19-2018 by F. Simoes                         !
!                                                                             !
!-----------------------------------------------------------------------------!
    ...
    IF (TRIM(basename) == 'iRIC') THEN  ! iRIC base name.
      ...
      DO j = 1,nzones
        ...
        IF (TRIM(zonename) == 'iRICZone') THEN  ! iRIC zone name.
          ...
          IF (ierror /= 0) THEN
            CALL cg_error_print_f()
            CALL byebye('SToRM ERROR: error writing ZVTX (Bed Elevation)to CGNS file.')
          END IF
          call cg_field_write_f(findex,i,j,fsol_iter,RealDouble,'Elevation',zvtx,fsol_id,ierror)
          IF (ierror /= 0) THEN
            CALL cg_error_print_f()
            CALL byebye('SToRM ERROR: error writing ZVTX (Elevation) to CGNS file.')
          END IF
